...
import { Badge } from "@/chat-ui/components/ui/badge";
import type { BadgeColor } from "@/chat-ui/components/ui/badge";

// ─── Shared collapsible parts ───────────────────────────────────────────────
//
// ThinkingSteps and ThinkingStepDetails are both single collapsible sections,
// built directly on Base UI's Collapsible (Root/Trigger/Panel) with
// framer-motion springs layered on top.
 ...
 * chevron that rotates from right (closed) to down (open).
 */
    ...
    return (
      ...
      >
        ...
        <Collapsible.Trigger
          ref={ref}
          className={cn(
            `relative z-10 flex items-center gap-2.5 ${shape.item} border-0 px-3 py-2 cursor-pointer outline-none select-none`,
...
function CollapsePanel({ open, children }: CollapsePanelProps) {
  ...
  // animation overshoots to scale× the real height and snaps back when the
  // final "auto" lands. offsetHeight and ResizeObserver are transform-immune.
      ...
      <CollapsePanel open={open}>
        <div className="flex flex-col gap-0.5 pt-0.5">
          {details?.map((item, i) => (
            <span
              key={i}
              className="text-[13px] text-muted-foreground leading-snug break-words whitespace-pre-wrap"
            >
              {item}
            </span>
          ))}
